...
/-! # Compact cutoff source assembly including short matrices
...
theorem one_or_fullBlock_cutoff_limit_of_section3_inputs_ae
  ...
  filter_upwards [cyclicBlock_all_interval_lengths_of_source_inputs_ae H lo hi hlo hfit b hb,
    one_or_fullBlock_expected_cutoff_limit_of_interval_lengths q H m₀ lo hi len hm₀ hmin hwindow hsize
...
theorem one_or_paperBand_cutoff_limit_of_section3_inputs_ae
    (q H m₀ lo hi d : ℕ → ℕ) (len : ∀ n, Fin (q n) → ℕ)
    [∀ n j, NeZero (len n j)] [∀ n, NeZero (∑ j, len n j)]
    (hm₀ : ∀ n, 0 < m₀ n)
    (hmin : ∀ n, q n = 1 ∨ ∀ j, m₀ n ≤ len n j)
    (hlo : ∀ n, 0 < lo n) (hfit : ∀ n, 2 * H n + 1 ≤ lo n)
  ...
  filter_upwards [one_or_fullBlock_cutoff_limit_of_section3_inputs_ae q H m₀ lo hi len hm₀ hmin hlo hfit hwindow hsize b hb hratio ha,
