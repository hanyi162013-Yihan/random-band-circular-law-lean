import SubgaussianSection8.CellResetLoss
...
def completeCellSiteEmbedding (Ξ : Atom) (c K : ℕ) (k : Fin K) : Fin c ↪ Fin (K * c) where
  toFun := completeCellSite k
  inj' := by
    intro i j hij
    exact (Prod.mk.inj (finProdFinEquiv.injective hij)).2

theorem intervalRestriction_flatten_cell (Ξ : Atom) (W c K : ℕ) (k : Fin K)
    (x : Fin K → IntervalRows W c) :
    intervalRestriction ((completeCellSiteEmbedding Ξ) c K k) (flattenCompleteCells W c K x) =
      x k := by
  funext i
  obtain ⟨⟨j, a⟩, rfl⟩ := finProdFinEquiv.surjective i
  change intervalRestriction ((completeCellSiteEmbedding Ξ) c K k)
  ...
  exact (flattenCompleteCells_row Ξ) W c K x k j a

theorem completeCell_good_of_flatten_good (Ξ : Atom) (I : NguyenBottomSingularInput.{0, 0})
    (W c K : ℕ) (x : Fin K → IntervalRows W c)
    (hx : flattenCompleteCells W c K x ∈ (subgaussianInterfaceGoodEvent Ξ) I W (K * c))
    (k : Fin K) : x k ∈ (subgaussianInterfaceGoodEvent Ξ) I W c := by
  have h := (subgaussianInterfaceGoodEvent_subset_subinterval Ξ) I ((completeCellSiteEmbedding Ξ) c K k) hx
  change intervalRestriction ((completeCellSiteEmbedding Ξ) c K k)
    (flattenCompleteCells W c K x) ∈ (subgaussianInterfaceGoodEvent Ξ) I W c at h
  rwa [(intervalRestriction_flatten_cell Ξ)] at h

def cellTransferBudget (Ξ : Atom) (I : NguyenBottomSingularInput.{0, 0}) (W s : ℕ) (z : ℂ) : ℝ :=
  (subgaussianTransferLogConstant Ξ) I z * ((s * W : ℕ) : ℝ) *
    (1 + Real.posLog (W : ℝ))

theorem interval_hodgeLoss_le_cellTransferBudget (Ξ : Atom)
...
theorem interval_product_det_isUnit_of_good (Ξ : Atom)
  ...
  have hd := (subgaussianInterface_dets_isUnit_of_good Ξ) I hI W s hW x hx z
  exact intervalClearedProduct_det_isUnit W s z x (fun j => (hd j).1) (fun j => (hd j).2) r

theorem clippedCoreLog_eq_log_on_good (Ξ : Atom)
    (I : NguyenBottomSingularInput.{0, 0}) (hI : Ξ.parameter ≤ I.subgaussianBound)
    (W s : ℕ) (hW : interfaceCanonicalLargeWThreshold I ≤ W)
    (z : ℂ) (x : IntervalRows W s) (hx : x ∈ (subgaussianInterfaceGoodEvent Ξ) I W s)
    (A : ℝ≥0) (hA : (cellTransferBudget Ξ) I W s z ≤ A) (r : Fin (2 * W + 1)) :
    clippedCoreLog A W s z r x = Real.log ‖intervalClearedProduct W s z x r‖ := by
  have hn := (subgaussianInterval_norm_pos_of_good Ξ) I hI W s hW z x hx r
  have hb := ((subgaussianInterval_abs_logNorm_le_on_measurable_good Ξ) I hI W s hW z x hx r).trans hA
  exact clippedLog_eq_log_of_log_bounds hn (abs_le.mp hb).1 (abs_le.mp hb).2

def optimizingCellResetLoss (Ξ : Atom) (μ : Measure ℝ) (A : ℝ≥0) (W s K : ℕ) (z : ℂ) (T : ℝ)
    (x : Fin K → IntervalRows W (3 + s)) : ℝ :=
  ∑ k, (cellIntervalResetLoss Ξ) W s K k z (clippedCoreOptimizingDegree μ A W s z) T x
...
theorem complete_cell_pressure_sandwich (Ξ : Atom)
    (I : NguyenBottomSingularInput.{0, 0}) (hI : Ξ.parameter ≤ I.subgaussianBound)
    (μ : Measure ℝ) (W s K : ℕ) (hW : interfaceCanonicalLargeWThreshold I ≤ W)
    (hs : 0 < s) (z : ℂ) (x : Fin K → IntervalRows W (3 + s))
    (hx : flattenCompleteCells W (3 + s) K x ∈ (subgaussianInterfaceGoodEvent Ξ) I W (K * (3 + s)))
    (A : ℝ≥0) (hA : (cellTransferBudget Ξ) I W s z ≤ A) (T : ℝ)
    (hT : 2 * (cellTransferBudget Ξ) I W s z + (cellTransferBudget Ξ) I W 3 z ≤ T) :
    (K : ℝ) * clippedMaxCorePressure μ A W s z - completeCellCoreFluctuation μ A W s K z x -
        (optimizingCellResetLoss Ξ) μ A W s K z T x ≤
      intervalMaxDegreeLog W (K * (3 + s)) z (flattenCompleteCells W (3 + s) K x) ∧
    intervalMaxDegreeLog W (K * (3 + s)) z (flattenCompleteCells W (3 + s) K x) ≤
      (K : ℝ) * clippedMaxCorePressure μ A W s z + completeCellCoreFluctuation μ A W s K z x +
        (K : ℝ) * (cellTransferBudget Ξ) I W 3 z := by
  have hcgood (k : Fin K) : completeCellCore W s (x k) ∈ (subgaussianInterfaceGoodEvent Ξ) I W s :=
    (subgaussianInterfaceGoodEvent_subset_subinterval Ξ) I (Fin.natAddEmb 3)
      ((completeCell_good_of_flatten_good Ξ) I W (3 + s) K x hx k)
  have hrgood (k : Fin K) : completeCellReset W s (x k) ∈ (subgaussianInterfaceGoodEvent Ξ) I W 3 :=
    (subgaussianInterfaceGoodEvent_subset_subinterval Ξ) I (Fin.castAddEmb s)
      ((completeCell_good_of_flatten_good Ξ) I W (3 + s) K x hx k)
  ...
  have hlower (r : Fin (2 * W + 1)) :
      (∑ k, clippedCoreLog A W s z r (completeCellCore W s (x k))) -
        (∑ k, (cellIntervalResetLoss Ξ) W s K k z r T x) ≤
      intervalDegreeLog W (K * (3 + s)) z r (flattenCompleteCells W (3 + s) K x) := by
    letI := hnonempty r
    have h := resetPrefixProduct_log_lower ((cellCoreProducts Ξ) W s K z r x)
      ((cellResetProducts Ξ) W s K z r x) K T (hunitc r) (hunitr r) (hcap r)
    rw [← (intervalClearedProduct_flatten_eq_resetPrefixProduct Ξ)] at h
    simp only [← Fin.sum_univ_eq_sum_range, (cellCoreProducts_val Ξ),
      (prefixResetLoss_eq_cellIntervalResetLoss Ξ)] at h
  ...
  have hupper (r : Fin (2 * W + 1)) :
      intervalDegreeLog W (K * (3 + s)) z r (flattenCompleteCells W (3 + s) K x) ≤
        (∑ k, clippedCoreLog A W s z r (completeCellCore W s (x k))) +
          (K : ℝ) * (cellTransferBudget Ξ) I W 3 z := by
    letI := hnonempty r
    have h := resetPrefixProduct_log_upper ((cellCoreProducts Ξ) W s K z r x)
      ((cellResetProducts Ξ) W s K z r x) K (hunitc r) (hunitr r)
    rw [← (intervalClearedProduct_flatten_eq_resetPrefixProduct Ξ)] at h
    simp only [← Fin.sum_univ_eq_sum_range, (cellCoreProducts_val Ξ), (cellResetProducts_val Ξ)] at h
    have hreset : (∑ k, Real.log ‖intervalClearedProduct W 3 z (completeCellReset W s (x k)) r‖) ≤
        (K : ℝ) * (cellTransferBudget Ξ) I W 3 z := by
      calc
        _ ≤ ∑ _k : Fin K, (cellTransferBudget Ξ) I W 3 z := by
          apply Finset.sum_le_sum
          intro k _
          exact (le_abs_self _).trans
            ((subgaussianInterval_abs_logNorm_le_on_measurable_good Ξ) I hI W 3 hW z _ (hrgood k) r)
        _ = _ := by simp
    have h' : Real.log ‖intervalClearedProduct W (K * (3 + s)) z
        ...
        (∑ k : Fin K, Real.log ‖intervalClearedProduct W s z (completeCellCore W s (x k)) r‖) +
          (K : ℝ) * (cellTransferBudget Ξ) I W 3 z :=
