import SubgaussianSection8.CellResetRates
...
def completeCellPressureError (Ξ : Atom) (I : NguyenBottomSingularInput.{0, 0})
  ...
  (intervalMaxDegreeLog W (K * (3 + coreSites W)) z
    ...
    (K : ℝ) * clippedMaxCorePressure Ξ.law
      (cellClipBound ((subgaussianCellClipConstant Ξ) I z) W) W (coreSites W) z) /
      ((K : ℝ) * cellLength W)

theorem completeCellPressureError_tendsto (Ξ : Atom)
    ...
    TendstoInProbabilityTri
      ...
      (fun n => (completeCellPressureError Ξ) I (W n) (K n) z) 0 := by
  let μ := fun n => independentCoreLaw Ξ.law (W n) (3 + coreSites (W n)) (K n)
  let A := fun n => cellClipBound ((subgaussianCellClipConstant Ξ) I z) (W n)
  let F := fun n x => completeCellCoreFluctuation Ξ.law (A n)
    (W n) (coreSites (W n)) (K n) z x / ((K n : ℝ) * cellLength (W n))
  let R := fun n x => (optimizingCellResetLoss Ξ) Ξ.law (A n)
    (W n) (coreSites (W n)) (K n) z ((subgaussianResetCap Ξ) I (W n) z) x /
      ((K n : ℝ) * cellLength (W n))
  let b := fun n => 3 * (subgaussianTransferLogConstant Ξ) I z *
    (densityLogScale (W n) / cellSites (W n))
  let E := fun n => (flattenCompleteCells (W n) (3 + coreSites (W n)) (K n)) ⁻¹'
    ((subgaussianInterfaceGoodEvent Ξ) I (W n) (K n * (3 + coreSites (W n))))ᶜ
  have hKpos : ∀ᶠ n in atTop, 0 < K n := hK.mono fun n hn =>
    (coreSites_pos (hW n)).trans_le hn
  have hF : TendstoInProbabilityTri μ F 0 :=
    completeCellCoreFluctuation_tendsto Ξ.law _ ((subgaussianCellClipConstant_pos Ξ) I z)
      W K hWtop hK z
  have hR : TendstoInProbabilityTri μ R 0 :=
    (normalizedCellResetLoss_tendsto Ξ) cook I hI W K m hW hWtop hKpos hm hlog z
      (fun n => clippedCoreOptimizingDegree Ξ.law (A n) (W n) (coreSites (W n)) z)
  ...
  have hE : Tendsto (fun n => (μ n).real (E n)) atTop (𝓝 0) := by
    ...
    have h := (measurePreserving_real_preimage_le
      (flattenCompleteCells_measurePreserving Ξ.law (W n) (3 + coreSites (W n)) (K n))
      ((subgaussianInterfaceGoodEvent Ξ) I (W n) (K n * (3 + coreSites (W n))))ᶜ).trans
        ((subgaussianInterfaceGoodEvent_compl_probability_le Ξ) I hI
  ...
  have hxg : flattenCompleteCells (W n) (3 + coreSites (W n)) (K n) x ∈
      (subgaussianInterfaceGoodEvent Ξ) I (W n) (K n * (3 + coreSites (W n))) := by
    simpa only [E, Set.mem_preimage, Set.mem_compl_iff, not_not] using hx
  have hs := (complete_cell_pressure_sandwich Ξ) I hI Ξ.law (W n) (coreSites (W n))
    (K n) hWn (coreSites_pos (hW n)) z x hxg (A n)
    ((subgaussianCellClipBound_ge_budget Ξ) I (W n) (hW n) z hlogn)
    ((subgaussianResetCap Ξ) I (W n) z) ((subgaussianResetCap_ge_budget Ξ) I (W n) (hW n) z)
  have hden : (0 : ℝ) < (K n : ℝ) * cellLength (W n) :=
    mul_pos (by exact_mod_cast hKn) (by exact_mod_cast cellLength_pos (hW n))
  have hfn : 0 ≤ completeCellCoreFluctuation Ξ.law (A n)
      (W n) (coreSites (W n)) (K n) z x := completeCellCoreFluctuation_nonneg _ _ _ _ _ _ _
  have hrn : 0 ≤ (optimizingCellResetLoss Ξ) Ξ.law (A n)
      (W n) (coreSites (W n)) (K n) z ((subgaussianResetCap Ξ) I (W n) z) x :=
    Finset.sum_nonneg fun j _ => cappedSpliceLoss_nonneg
      ((subgaussianResetCap_pos Ξ) I (W n) (hW n) z).le _ _ _
  have hbn : 0 ≤ b n := mul_nonneg
    (mul_nonneg (by norm_num) ((subgaussianTransferLogConstant_nonneg Ξ) I z))
    (div_nonneg (densityLogScale_nonneg (hW n)) (Nat.cast_nonneg _))
  have hbudget : (K n : ℝ) * (cellTransferBudget Ξ) I (W n) 3 z /
...
theorem intervalCompleteCellPressureError_tendsto (Ξ : Atom)
    (cook : CookInput Ξ) (I : NguyenBottomSingularInput.{0, 0})
    (hI : Ξ.parameter ≤ I.subgaussianBound) (W K m : ℕ → ℕ)
    (hW : ∀ n, 0 < W n) (hWtop : Tendsto W atTop atTop)
    (hK : ∀ᶠ n in atTop, coreSites (W n) ≤ K n)
    ...
    (z : ℂ) :
    TendstoInProbabilityTri
      (fun n => intervalRowsLaw (W n) (K n * (3 + coreSites (W n))) Ξ.law)
      (fun n x => (completeCellPressureError Ξ) I (W n) (K n) z
  ...
  exact tendstoInProbabilityTri_comp_measurePreserving _ _
    ...
    (fun n => unflattenCompleteCells_measurePreserving Ξ.law
      (W n) (3 + coreSites (W n)) (K n)) _ 0
    ((completeCellPressureError_tendsto Ξ) cook I hI W K m hW hWtop hK hm hlog z)

/-- The complete-cell pressure compared on any ambient physical interval,
normalized by the ambient scalar dimension. -/
def embeddedCompleteCellPressureError (Ξ : Atom) (I : NguyenBottomSingularInput.{0, 0})
...
theorem embeddedCompleteCellPressureError_tendsto (Ξ : Atom)
    (cook : CookInput Ξ) (I : NguyenBottomSingularInput.{0, 0})
    (hI : Ξ.parameter ≤ I.subgaussianBound) (W K m : ℕ → ℕ)
    (hW : ∀ n, 0 < W n) (hWtop : Tendsto W atTop atTop)
    (hK : ∀ᶠ n in atTop, coreSites (W n) ≤ K n)
    ...
    TendstoInProbabilityTri (fun n => intervalRowsLaw (W n) (m n) Ξ.law)
      (fun n => (embeddedCompleteCellPressureError Ξ) I (W n) (K n) (m n) z (e n)) 0 := by
  let μ := fun n => intervalRowsLaw (W n) (m n) Ξ.law
  have h := tendstoInProbabilityTri_comp_measurePreserving μ
    (fun n => intervalRowsLaw (W n) (K n * (3 + coreSites (W n))) Ξ.law)
    (fun n => intervalRestriction (W := W n) (e n))
    (fun n => physicalRestriction_measurePreserving Ξ.law (e n)) _ 0
    ((intervalCompleteCellPressureError_tendsto Ξ) cook I hI W K m hW hWtop hK hm hlog z)
