...
def physicalCappedResetLoss (Ξ : Atom) (W p q : ℕ) (z : ℂ) (r : Fin (2 * W + 1)) (T : ℝ)
...
theorem measurable_physicalCappedResetLoss (Ξ : Atom) (W p q : ℕ) (z : ℂ)
    (r : Fin (2 * W + 1)) (T : ℝ)
    (core : IntervalRows W p) (past : IntervalRows W q) :
    Measurable ((physicalCappedResetLoss Ξ) W p q z r T core past) := by
...
theorem physicalCappedResetLoss_nonneg (Ξ : Atom) (W p q : ℕ) (z : ℂ)
    (r : Fin (2 * W + 1)) {T : ℝ} (hT : 0 ≤ T)
    (core : IntervalRows W p) (past : IntervalRows W q) (reset : IntervalRows W 3) :
    0 ≤ (physicalCappedResetLoss Ξ) W p q z r T core past reset :=
  cappedSpliceLoss_nonneg hT _ _ _

theorem physicalCappedResetLoss_le_cap (Ξ : Atom) (W p q : ℕ) (z : ℂ)
    (r : Fin (2 * W + 1)) (T : ℝ)
    (core : IntervalRows W p) (past : IntervalRows W q) (reset : IntervalRows W 3) :
    (physicalCappedResetLoss Ξ) W p q z r T core past reset ≤ T :=
...
theorem physicalCappedResetLoss_fresh_integral_le (Ξ : Atom)
    (cook : CookInput Ξ) (W p q : ℕ) (z : ℂ)
    ...
    (hGamma : -Real.log ((subgaussianBoundaryInverseGamma Ξ) W z
    ...
    (∫ x, (physicalCappedResetLoss Ξ) W p q z r T core past
      (packetPhysicalRows W (ep, x)) ∂packetAtomRowsLaw W Ξ.law) ≤
        D + (subgaussianBoundaryBaseLoss Ξ) cook W z +
          (subgaussianBoundaryBadProbability Ξ) cook W * T := by
  have hW0 : 0 < W := by
  ...
  let c := (subgaussianPacketFrameCoefficient Ξ) W r.1 z
    (normalizedBlockMatrix W ep.1) (normalizedBlockMatrix W ep.2) U V s
  have hc := (subgaussianPacketFrameCoefficient_lower_and_pos Ξ) W r.1 hW0 z
    (normalizedBlockMatrix W ep.1) (normalizedBlockMatrix W ep.2) hCL hBR U V s
  have hGamma0 := (subgaussianBoundaryInverseGamma_pos Ξ) W hW0 z
  ...
  have hpacket : Measurable (fun x : PacketAtomRows W => packetPhysicalRows W (ep, x)) :=
    (packetPhysicalRows_measurePreserving (μ := Ξ.law) W).measurable.comp
      (measurable_const.prodMk measurable_id)
  apply integral_cappedSpliceLoss_le (packetAtomRowsLaw W Ξ.law)
  ...
  · exact (subgaussianPhysicalPacketCoefficient_capped Ξ) cook W z hW ep hCL hBR r U V s T hT
