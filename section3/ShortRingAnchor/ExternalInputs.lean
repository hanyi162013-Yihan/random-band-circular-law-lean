...
`rate_n=M^{-zeta}` in the source. A proved constructor from compact
Stieltjes control is `lemma35LocalBulkComparisonInput_of_stieltjes` in
`PoissonSmoothingProbability.lean`; no density interface is used there. -/
