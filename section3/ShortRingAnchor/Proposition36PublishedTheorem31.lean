...
import ShortRingAnchor.Theorem31CyclicReal

/-!
# Proposition 3.6 with the published Theorem 3.1 and Hermitization count connected

The copied Theorem 3.1 handles both density alternatives. Its real branch
retains the explicit geometric Brascamp--Lieb premise. BBV and the two BC12
conclusions remain explicit; no least-value or counting conclusion is assumed.
-/
...
theorem proposition36_cyclicShortRing_from_published_theorem31
  ...
  have hbeta : v3BandwidthExponent omega ≤ 1 := by
    ...
    linarith [homega.2]
  obtain ⟨goodLSV, hLSV⟩ := theorem31MinimumSingularValueInput_cyclic_of_densityAlternative weights hfit hMpos hM
    ringEntry atomA hatomA hcopiesA hDensityA hGBL
