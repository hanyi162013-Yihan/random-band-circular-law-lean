/- Reused proof of the subgaussian norm event in Proposition 3.8.
Source: random-band-circular-law-lean, Section9/BernoulliSection9/
SubgaussianOperatorNorm.lean, original SHA256
8de1f4ff8fcb53889ef4d7fa2f0ae9b0c2fa12d8e20fc999d5db72239caf1978.
Only the namespace and the model-only import are changed. In particular,
this module has no Cook, Nguyen, or Proposition 3.8 estimate as a premise. -/
...
def rawRealMatrix
    ...
    (S : SubgaussianNorm.IidSubgaussianSquare Ω μ n) (ω : Ω) :
    Matrix (Fin n) (Fin n) ℝ := fun i j ↦ S.atom (i, j) ω

def realBilinear
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} {n : ℕ}
    (S : SubgaussianNorm.IidSubgaussianSquare Ω μ n)
    (x y : E n) (ω : Ω) : ℝ :=
  ∑ p : Fin n × Fin n, y p.1 * x p.2 * S.atom p ω

def subgaussianEnvelope
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} {n : ℕ}
    (S : SubgaussianNorm.IidSubgaussianSquare Ω μ n) : ℝ≥0 :=
...
theorem rawRealMatrix_opNorm_tail
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} {n : ℕ}
    (S : SubgaussianNorm.IidSubgaussianSquare Ω μ n) (hn : 0 < n) :
...
def subgaussianOpNormConstant
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} {n : ℕ}
    (S : SubgaussianNorm.IidSubgaussianSquare Ω μ n) : ℝ :=
  40 * Real.sqrt (subgaussianEnvelope S : ℝ)

def subgaussianOpNormBadEvent
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} {n : ℕ}
    (S : SubgaussianNorm.IidSubgaussianSquare Ω μ n) : Set Ω :=
