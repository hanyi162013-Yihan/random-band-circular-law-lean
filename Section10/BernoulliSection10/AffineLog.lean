...
certificate.  The proof uses only the hypotheses in `IsBoundedDensityAtom`.
-/
...
def affineValue {p : ℕ} {E : Type*} [AddCommMonoid E] [Module ℝ E]
    (v₀ : E) (v : Fin p → E) (x : Fin p → ℝ) : E :=
  v₀ + ∑ i, x i • v i
...
variable {p : ℕ} {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
...
theorem norm_affineValue_le (v₀ : E) (v : Fin p → E) (x : Fin p → ℝ) :
...
theorem norm_affineValue_le_piNorm (v₀ : E) (v : Fin p → E) (x : Fin p → ℝ) :
    ‖affineValue v₀ v x‖ ≤ ‖v₀‖ + (∑ i, |x i|) * ‖v‖ := by
  refine (norm_affineValue_le v₀ v x).trans (add_le_add (le_refl _) ?_)
  rw [Finset.sum_mul]
  exact Finset.sum_le_sum fun i _ =>
    mul_le_mul_of_nonneg_left (norm_le_pi_norm v i) (abs_nonneg _)

theorem norm_affineValue_le_sup (v₀ : E) (v : Fin p → E) (x : Fin p → ℝ) :
...
theorem norm_affineValue_le_lambda_mul (v₀ : E) (v : Fin p → E) (x : Fin p → ℝ) :
...
theorem measure_pi_norm_affineValue_le (hμ : IsBoundedDensityAtom μ L)
    ...
    (Measure.pi fun _ : Fin (n + 1) => μ)
        {x : Fin (n + 1) → ℝ | ‖affineValue v₀ v x‖ ≤ r} ≤
  ...
  have hA : MeasurableSet A := by
    exact measurableSet_le
      ...
      measurable_const
  have hevent :
      {x : Fin (n + 1) → ℝ | ‖affineValue v₀ v x‖ ≤ r} = e ⁻¹' A := by
    ext x
    change ‖affineValue v₀ v x‖ ≤ r ↔
      ‖affineValue v₀ v (e.symm (e x))‖ ≤ r
    rw [e.symm_apply_apply]
  have hpush := congrArg (fun m : Measure (ℝ × (Fin n → ℝ)) => m A)
...
theorem measure_affineValue_large_center_markov_le (hμ : IsBoundedDensityAtom μ L)
  ...
  have hsubset :
      {x : Fin (n + 1) → ℝ | ‖affineValue v₀ v x‖ ≤ u * b} ⊆
        {x : Fin (n + 1) → ℝ | b / (2 * p * ν) ≤ ‖x‖} := by
    intro x hx
...
/-- Explicit layer-cake estimate for an exponentially-tailed nonnegative
random variable.  This helper is independent of the affine application. -/
theorem lintegral_sq_le_of_exponential_tail (ν : Measure Ω) [IsProbabilityMeasure ν]
    (Z : Ω → ℝ) (hZ : Measurable Z) (hZnonneg : ∀ ω, 0 ≤ Z ω)
    (B : ℝ) (hB : 1 ≤ B)
    (htail : ∀ t : ℝ, 0 ≤ t →
      ν {ω | t < Z ω} ≤ ENNReal.ofReal (B * Real.exp (-t / 2))) :
    ∫⁻ ω, ENNReal.ofReal ((Z ω) ^ 2) ∂ν ≤
      ENNReal.ofReal (16 * Real.log B ^ 2 + 32) := by
  let T : ℝ := 4 * Real.log B
  have hBpos : 0 < B := zero_lt_one.trans_le hB
  have hlogB : 0 ≤ Real.log B := Real.log_nonneg hB
  have hT : 0 ≤ T := by simp [T, hlogB]
  have hgint : ∀ s : ℝ, 0 < s →
      IntervalIntegrable (fun t : ℝ => 2 * t) volume 0 s := by
    intro s _
    exact ((continuous_const.mul continuous_id :
      Continuous (fun t : ℝ => 2 * t))).intervalIntegrable (μ := volume) 0 s
  have hgnonneg :
      ∀ᵐ t ∂(volume.restrict (Ioi (0 : ℝ))), 0 ≤ (2 : ℝ) * t := by
    filter_upwards [ae_restrict_mem measurableSet_Ioi] with t ht
    exact mul_nonneg (by norm_num) ht.le
  have hcake := lintegral_comp_eq_lintegral_meas_lt_mul ν
    (ae_of_all ν hZnonneg) hZ.aemeasurable hgint hgnonneg
  have hprimitive : ∀ ω,
      ∫ t in (0 : ℝ)..Z ω, 2 * t = (Z ω) ^ 2 := by
    intro ω
    rw [intervalIntegral.integral_const_mul, integral_id]
    ring
  simp_rw [hprimitive] at hcake
  rw [hcake, ← Ioc_union_Ioi_eq_Ioi hT,
    lintegral_union measurableSet_Ioi Ioc_disjoint_Ioi_same]
  have hhead :
      ∫⁻ t in Ioc 0 T, ν {ω | t < Z ω} * ENNReal.ofReal (2 * t) ≤
        ENNReal.ofReal (T ^ 2) := by
    calc
      ∫⁻ t in Ioc 0 T, ν {ω | t < Z ω} * ENNReal.ofReal (2 * t) ≤
          ∫⁻ t in Ioc 0 T, ENNReal.ofReal (2 * t) := by
        apply setLIntegral_mono' measurableSet_Ioc
        intro t ht
        have hm : ν {ω | t < Z ω} ≤ 1 := by
          calc
            ν {ω | t < Z ω} ≤ ν univ := measure_mono (subset_univ _)
            _ = 1 := measure_univ
        simpa using mul_le_mul_left hm (ENNReal.ofReal (2 * t))
      _ = ENNReal.ofReal (T ^ 2) := by
        have hint : IntegrableOn (fun t : ℝ => 2 * t) (Ioc 0 T) :=
          ((continuous_const.mul continuous_id :
            Continuous (fun t : ℝ => 2 * t))).integrableOn_Ioc
        have hnonneg : ∀ᵐ t ∂(volume.restrict (Ioc 0 T)), 0 ≤ (2 : ℝ) * t := by
          filter_upwards [ae_restrict_mem measurableSet_Ioc] with t ht
          exact mul_nonneg (by norm_num) ht.1.le
        calc
          ∫⁻ t in Ioc 0 T, ENNReal.ofReal (2 * t) =
              ENNReal.ofReal (∫ t in Ioc 0 T, 2 * t) :=
          ...
          _ = ENNReal.ofReal (T ^ 2) := by
            ...
            ring
  have htailPoint : ∀ t ∈ Ioi T,
      ν {ω | t < Z ω} * ENNReal.ofReal (2 * t) ≤
        ENNReal.ofReal (8 * B * Real.exp (-t / 4)) := by
    intro t htI
    have ht0 : 0 ≤ t := hT.trans htI.le
    have htlinear : t ≤ 4 * Real.exp (t / 4) := by
      nlinarith [Real.add_one_le_exp (t / 4)]
    calc
      ν {ω | t < Z ω} * ENNReal.ofReal (2 * t) ≤
          ENNReal.ofReal (B * Real.exp (-t / 2)) * ENNReal.ofReal (2 * t) :=
        mul_le_mul_left (htail t ht0) _
      _ = ENNReal.ofReal (B * Real.exp (-t / 2) * (2 * t)) := by
        rw [ENNReal.ofReal_mul (by positivity : 0 ≤ B * Real.exp (-t / 2))]
      _ ≤ ENNReal.ofReal (8 * B * Real.exp (-t / 4)) := by
        apply ENNReal.ofReal_le_ofReal
        calc
          B * Real.exp (-t / 2) * (2 * t) =
              (2 * B * Real.exp (-t / 2)) * t := by ring
          _ ≤ (2 * B * Real.exp (-t / 2)) * (4 * Real.exp (t / 4)) := by
            exact mul_le_mul_of_nonneg_left htlinear (by positivity)
          _ = 8 * B * Real.exp (-t / 4) := by
            calc
              (2 * B * Real.exp (-t / 2)) * (4 * Real.exp (t / 4)) =
                  8 * B * (Real.exp (-t / 2) * Real.exp (t / 4)) := by ring
              _ = 8 * B * Real.exp (-t / 4) := by
                rw [← Real.exp_add]
                congr 2
                ring
  have htailInt :
      ∫⁻ t in Ioi T, ENNReal.ofReal (8 * B * Real.exp (-t / 4)) =
        ENNReal.ofReal 32 := by
    have hfun : (fun t : ℝ => 8 * B * Real.exp (-t / 4)) =
        fun t : ℝ => 8 * B * Real.exp ((-(1 : ℝ) / 4) * t) := by
      funext t
      congr 2
      ring
    have hint : IntegrableOn (fun t : ℝ => 8 * B * Real.exp (-t / 4)) (Ioi T) := by
      rw [hfun]
      exact (integrableOn_exp_mul_Ioi (a := (-(1 : ℝ) / 4)) (by norm_num) T).const_mul _
    rw [← ofReal_integral_eq_lintegral_ofReal hint]
    · rw [hfun, integral_const_mul,
          integral_exp_mul_Ioi (a := (-(1 : ℝ) / 4)) (by norm_num) T]
      dsimp [T]
      rw [show (-(1 : ℝ) / 4) * (4 * Real.log B) = -Real.log B by ring,
        Real.exp_neg, Real.exp_log hBpos]
      field_simp
      ring
    · filter_upwards [ae_restrict_mem measurableSet_Ioi] with t ht
      positivity
  have htailPart :
      ∫⁻ t in Ioi T, ν {ω | t < Z ω} * ENNReal.ofReal (2 * t) ≤
        ENNReal.ofReal 32 := by
    exact (setLIntegral_mono' measurableSet_Ioi htailPoint).trans_eq htailInt
  calc
    (∫⁻ t in Ioc 0 T, ν {ω | t < Z ω} * ENNReal.ofReal (2 * t)) +
        ∫⁻ t in Ioi T, ν {ω | t < Z ω} * ENNReal.ofReal (2 * t) ≤
      ENNReal.ofReal (T ^ 2) + ENNReal.ofReal 32 := add_le_add hhead htailPart
    _ = ENNReal.ofReal (T ^ 2 + 32) := by
      rw [ENNReal.ofReal_add] <;> positivity
    _ = ENNReal.ofReal (16 * Real.log B ^ 2 + 32) := by
      congr 2
      dsimp [T]
      ring

/-- Adding a deterministic real center costs at most the elementary
`(a+c)² ≤ 2a²+2c²` bound.  The probability normalization makes the
constant term integrate to itself. -/
theorem lintegral_sq_add_const_le (ν : Measure Ω) [IsProbabilityMeasure ν]
    (f : Ω → ℝ) (hf : Measurable f) (c K : ℝ) (hK : 0 ≤ K)
    (hI : ∫⁻ ω, ENNReal.ofReal (f ω ^ 2) ∂ν ≤ ENNReal.ofReal K) :
    ∫⁻ ω, ENNReal.ofReal ((f ω + c) ^ 2) ∂ν ≤
      ENNReal.ofReal (2 * K + 2 * c ^ 2) := by
  let F : Ω → ℝ≥0∞ := fun ω ↦ ENNReal.ofReal (f ω ^ 2)
  have hF : Measurable F := (hf.pow measurable_const).ennreal_ofReal
  calc
    (∫⁻ ω, ENNReal.ofReal ((f ω + c) ^ 2) ∂ν) ≤
        ∫⁻ ω, ENNReal.ofReal (2 * f ω ^ 2 + 2 * c ^ 2) ∂ν := by
      apply lintegral_mono
      intro ω
      apply ENNReal.ofReal_le_ofReal
      nlinarith [sq_nonneg (f ω - c)]
    _ = ∫⁻ ω, ENNReal.ofReal 2 * F ω + ENNReal.ofReal (2 * c ^ 2) ∂ν := by
    ...
    _ = ENNReal.ofReal 2 * (∫⁻ ω, F ω ∂ν) +
        ENNReal.ofReal (2 * c ^ 2) := by
      calc
        (∫⁻ ω, ENNReal.ofReal 2 * F ω + ENNReal.ofReal (2 * c ^ 2) ∂ν) =
            (∫⁻ ω, ENNReal.ofReal 2 * F ω ∂ν) +
              ∫⁻ _ω, ENNReal.ofReal (2 * c ^ 2) ∂ν :=
          lintegral_add_left (measurable_const.mul hF) _
        _ = ENNReal.ofReal 2 * (∫⁻ ω, F ω ∂ν) +
            ENNReal.ofReal (2 * c ^ 2) := by
          rw [lintegral_const_mul _ hF]
          simp
    _ ≤ ENNReal.ofReal 2 * ENNReal.ofReal K + ENNReal.ofReal (2 * c ^ 2) := by
      apply add_le_add
      · exact mul_le_mul_right hI _
      · exact le_rfl
    _ = ENNReal.ofReal (2 * K + 2 * c ^ 2) := by
      calc
        ENNReal.ofReal 2 * ENNReal.ofReal K + ENNReal.ofReal (2 * c ^ 2) =
            ENNReal.ofReal (2 * K) + ENNReal.ofReal (2 * c ^ 2) := by
          rw [← ENNReal.ofReal_mul (by norm_num : (0 : ℝ) ≤ 2)]
        _ = ENNReal.ofReal (2 * K + 2 * c ^ 2) :=
          (ENNReal.ofReal_add (mul_nonneg (by norm_num) hK)
            (mul_nonneg (by norm_num) (sq_nonneg c))).symm

/-- The squared difference of two independent copies costs a factor four. -/
theorem lintegral_lintegral_sq_sub_le (ν : Measure Ω) [IsProbabilityMeasure ν]
    (f : Ω → ℝ) (hf : Measurable f) (K : ℝ) (hK : 0 ≤ K)
    (hI : ∫⁻ ω, ENNReal.ofReal (f ω ^ 2) ∂ν ≤ ENNReal.ofReal K) :
    ∫⁻ ω, ∫⁻ ω', ENNReal.ofReal ((f ω - f ω') ^ 2) ∂ν ∂ν ≤
      ENNReal.ofReal (4 * K) := by
  let F : Ω → ℝ≥0∞ := fun ω ↦ ENNReal.ofReal (f ω ^ 2)
  have hF : Measurable F := (hf.pow measurable_const).ennreal_ofReal
  have hIneg : ∫⁻ ω, ENNReal.ofReal ((-f ω) ^ 2) ∂ν ≤ ENNReal.ofReal K := by
    simpa only [neg_sq] using hI
  have hinner : ∀ ω,
      (∫⁻ ω', ENNReal.ofReal ((f ω - f ω') ^ 2) ∂ν) ≤
        ENNReal.ofReal (2 * K + 2 * f ω ^ 2) := by
    intro ω
    simpa [sub_eq_add_neg, add_comm] using
      (lintegral_sq_add_const_le ν (fun ω' ↦ -f ω') hf.neg (f ω) K hK hIneg)
  calc
    (∫⁻ ω, ∫⁻ ω', ENNReal.ofReal ((f ω - f ω') ^ 2) ∂ν ∂ν) ≤
        ∫⁻ ω, ENNReal.ofReal (2 * K + 2 * f ω ^ 2) ∂ν :=
      lintegral_mono hinner
    _ = ∫⁻ ω, ENNReal.ofReal (2 * K) + ENNReal.ofReal 2 * F ω ∂ν := by
      apply lintegral_congr
      intro ω
      dsimp [F]
      rw [ENNReal.ofReal_add (mul_nonneg (by norm_num) hK)
        (mul_nonneg (by norm_num) (sq_nonneg _))]
      congr 1
      rw [ENNReal.ofReal_mul (by norm_num : (0 : ℝ) ≤ 2)]
    _ = ENNReal.ofReal (2 * K) + ENNReal.ofReal 2 * (∫⁻ ω, F ω ∂ν) := by
      calc
        (∫⁻ ω, ENNReal.ofReal (2 * K) + ENNReal.ofReal 2 * F ω ∂ν) =
            (∫⁻ _ω, ENNReal.ofReal (2 * K) ∂ν) +
              ∫⁻ ω, ENNReal.ofReal 2 * F ω ∂ν :=
          lintegral_add_left measurable_const _
        _ = ENNReal.ofReal (2 * K) + ENNReal.ofReal 2 * (∫⁻ ω, F ω ∂ν) := by
          rw [lintegral_const_mul _ hF]
          simp
    _ ≤ ENNReal.ofReal (2 * K) + ENNReal.ofReal 2 * ENNReal.ofReal K := by
      apply add_le_add le_rfl
      exact mul_le_mul_right hI _
    _ = ENNReal.ofReal (4 * K) := by
      rw [← ENNReal.ofReal_mul (by norm_num : (0 : ℝ) ≤ 2),
        ← ENNReal.ofReal_add (mul_nonneg (by norm_num) hK) (mul_nonneg (by norm_num) hK)]
      congr 2
      ring
...
theorem measurable_affineLogSqDifference {p : ℕ}
  ...
  have hlog : Measurable fun x : Fin p → ℝ ↦ Real.log ‖affineValue v₀ v x‖ :=
