...
theorem densityCyclicMatrix_from_square (W s : ℕ)
    (x : Fin ((s + 3) * W) × Fin ((s + 3) * W) → ℝ)
    (i j : Fin ((s + 3) * W)) :
    densityCyclicMatrix W s (physicalRowsFromSquare W s x) i j =
      (physicalProfile W s i j * x (i, j) : ℝ) := by
...
theorem densityCyclicMatrix_from_sequence (W s : ℕ) (ω : ℕ → ℝ)
    (i j : Fin ((s + 3) * W)) :
    densityCyclicMatrix W s (physicalRowsFromSequence W s ω) i j =
      (physicalProfile W s i j * ω (finProdFinEquiv (i, j)).val : ℝ) :=
...
theorem normalizedPhysicalAtom_norm_sq {W : ℕ} (x : PhysicalRowAtoms W)
  ...
  simp only [normalizedPhysicalAtom, Complex.norm_real, Real.norm_eq_abs,
    sq_abs, mul_pow, blockNormalization_sq]
