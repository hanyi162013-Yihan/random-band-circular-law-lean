...
abbrev BlockAtomRows (W : ℕ) := Fin W → Fin W → ℝ

def blockAtomRowsLaw (W : ℕ) (μ : Measure ℝ) : Measure (BlockAtomRows W) :=
...
theorem normalizedBlockDet_update_line
  ...
  let ru : Fin W → ℂ := fun b ↦
    Complex.ofReal (blockNormalization W * u b)
  let rv : Fin W → ℂ := fun b ↦
    Complex.ofReal (blockNormalization W * v b)
  have hrow :
      (fun b : Fin W ↦
        Complex.ofReal (blockNormalization W *
          (((1 - t) • u + t • v) b))) =
...
theorem replicatedOneHotRows_eq_finRowsToMultiAffineRows
    ...
    replicatedOneHotRows p n i =
      finRowsToMultiAffineRows p n
        (fun a ↦ (Pi.single (i a) 1 : Fin p → ℝ)) := by
  intro n
  induction n with
  | zero => intro i; rfl
  | succ n ih =>
      ...
      change (Pi.single (i 0) 1, replicatedOneHotRows p n (Fin.tail i)) =
        (Pi.single (i 0) 1,
          finRowsToMultiAffineRows p n
            (Fin.tail (fun a ↦ (Pi.single (i a) 1 : Fin p → ℝ))))
      congr 1
      rw [ih]
      congr 1
...
def endpointBlockPairLaw (W : ℕ) (μ : Measure ℝ) :
