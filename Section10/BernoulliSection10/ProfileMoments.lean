...
theorem inputAtom_measurePreserving (μ : Measure ℝ) [IsProbabilityMeasure μ] (k : ℕ) :
...
theorem integrable_profileMatrix_entry
  ...
  have hi := (inputAtom_measurePreserving μ (squareAtomIndex i j)).integrable_comp_of_integrable
    hμ.integrable_id
  exact (hi.const_mul (σ i j)).ofReal
...
theorem profileMatrix_norm_sq {N : ℕ}
  ...
  simp only [profileMatrix, Complex.norm_real, Real.norm_eq_abs, sq_abs, mul_pow]
...
theorem integral_profileMatrix_entry
  ...
  letI := hμ.toIsProbabilityMeasure
  have he := (real_integral_comp_measurePreserving
    (inputAtom_measurePreserving μ (squareAtomIndex i j)) measurable_id).trans hμ.centered
  change (∫ ω : InputSpace, ω.1 (squareAtomIndex i j) ∂inputLaw μ) = 0 at he
  simp only [profileMatrix, integral_complex_ofReal, integral_const_mul, he, mul_zero,
    Complex.ofReal_zero]
...
theorem integral_profileMatrix_norm_sq
  ...
  have he := (real_integral_comp_measurePreserving
    ...
    (by fun_prop : Measurable (fun x : ℝ => x ^ 2))).trans hμ.variance_one
