...
/-- A map on a real vector space which preserves every two-point affine
...
theorem affineValue_eq_of_map_line
  ...
  have hD_add (x y : Fin p → ℝ) : D (x + y) = D x + D y := by
    have h := hF ((2 : ℝ) • x) ((2 : ℝ) • y) (1 / 2 : ℝ)
    have hx := hD_smul (2 : ℝ) x
    have hy := hD_smul (2 : ℝ) y
    change F ((2 : ℝ) • x) - F 0 = (2 : ℝ) • (F x - F 0) at hx
    change F ((2 : ℝ) • y) - F 0 = (2 : ℝ) • (F y - F 0) at hy
    have hx' : F ((2 : ℝ) • x) = (2 : ℝ) • (F x - F 0) + F 0 :=
      (sub_eq_iff_eq_add).mp hx
    have hy' : F ((2 : ℝ) • y) = (2 : ℝ) • (F y - F 0) + F 0 :=
      (sub_eq_iff_eq_add).mp hy
    have hhalfsub : (1 - (1 / 2 : ℝ)) = 1 / 2 := by norm_num
    have htwo_half (u : E) : (1 / 2 : ℝ) • ((2 : ℝ) • u) = u := by
      rw [smul_smul]
      norm_num
    have hhalf_add (u : E) :
        (1 / 2 : ℝ) • u + (1 / 2 : ℝ) • u = u := by
      rw [← add_smul]
      norm_num
    ...
    dsimp [D]
    rw [h, hhalfsub, hx', hy']
    simp only [smul_add, htwo_half]
    calc
      ((F x - F 0) + (1 / 2 : ℝ) • F 0 +
            ((F y - F 0) + (1 / 2 : ℝ) • F 0)) - F 0 =
          (F x - F 0) + (F y - F 0) +
            (((1 / 2 : ℝ) • F 0 + (1 / 2 : ℝ) • F 0) - F 0) := by
        abel
      _ = (F x - F 0) + (F y - F 0) := by
        rw [hhalf_add]
        simp
  let d : (Fin p → ℝ) →ₗ[ℝ] E := {
    toFun := D
