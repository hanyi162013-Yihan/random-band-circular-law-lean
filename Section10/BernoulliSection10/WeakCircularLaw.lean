...
/-!
Adapted from the user's Section 5 `WeakCircularLaw.lean` (2026-09-02).
The proof is dimension-uniform; the index now allows arbitrary positive sizes.
...
cutoff equal to one there prevents escape of spectral mass. Thus the Section 10
...
variable {N : ℕ → ℕ} [∀ n, Nonempty (Fin (N n))]
...
theorem circularLaw_boundedContinuous_of_compactSupport
    {Ω : Type*} [MeasurableSpace Ω] (P : Measure Ω) [IsProbabilityMeasure P]
    (X : ∀ n, Ω → Matrix (Fin (N n)) (Fin (N n)) ℂ)
