...
open BernoulliSection10.Replacement TaoVuReplacement ShortRingAnchor
...
theorem diagonalDiskReference_logPotential_limit_sequence
    (d : ℕ → ℕ) (hd : Tendsto d atTop atTop) (z : ℂ) :
    TendstoInMeasure circularSampleMeasure
      (fun n ω => physicalLogPotential (diagonalDiskReference (d n) ω) z) atTop
      (fun _ => circularLogPotential z) := by
  rw [tendstoInMeasure_iff_norm]
  intro ε hε
  exact ((tendstoInMeasure_iff_norm.mp (diagonalDiskReference_logPotential_limit z)) ε hε).comp hd

theorem diagonalDiskReference_esd_limit_sequence
    (d : ℕ → ℕ) (hd : Tendsto d atTop atTop)
    (f : ℂ → ℝ) (hf : Continuous f) (hc : HasCompactSupport f) :
    TendstoInMeasure circularSampleMeasure
      (fun n ω => realEsdTest (diagonalDiskReference (d n) ω) f) atTop
      (fun _ => ∫ w, f w ∂circularMeasure) := by
  rw [tendstoInMeasure_iff_norm]
  intro ε hε
  exact ((tendstoInMeasure_iff_norm.mp (diagonalDiskReference_esd_limit f hf hc)) ε hε).comp hd
