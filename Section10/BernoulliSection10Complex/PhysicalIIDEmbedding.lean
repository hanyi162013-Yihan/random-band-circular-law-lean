import BernoulliSection10Complex.CyclicPhysicalModel
...
def physicalRowsFromSquare (W s : ℕ)
    (x : Fin ((s + 3) * W) × Fin ((s + 3) * W) → ℂ) : IntervalRows W (s + 3) :=
...
def squareIIDFromSequence (n : ℕ) (ω : ℕ → ℂ) : Fin n × Fin n → ℂ :=
  fun p => ω (finProdFinEquiv p).val

theorem squareIIDFromSequence_measurePreserving
    (μ : Measure ℂ) [IsProbabilityMeasure μ] (n : ℕ) :
...
def physicalRowsFromSequence (W s : ℕ) (ω : ℕ → ℂ) : IntervalRows W (s + 3) :=
