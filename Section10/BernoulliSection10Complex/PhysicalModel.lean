...
coordinate is a complete physical equation row with `3 * W` complex atoms.
...
/-- The `3W` independent complex atoms in one physical equation row. -/
abbrev PhysicalRowAtoms (W : ℕ) := Fin (3 * W) → ℂ

/-- The product law of one physical row. -/
def physicalRowLaw (W : ℕ) (μ : Measure ℂ) : Measure (PhysicalRowAtoms W) :=
...
def intervalRowsLaw (W s : ℕ) (μ : Measure ℂ) : Measure (IntervalRows W s) :=
...
def normalizedPhysicalAtom {W : ℕ} (x : PhysicalRowAtoms W)
    (b : Fin 3) (c : Fin W) : ℂ :=
  (blockNormalization W : ℂ) * x (physicalAtomIndex b c)

/-- The concrete physical row obtained from its `3W` complex atoms.  The middle
