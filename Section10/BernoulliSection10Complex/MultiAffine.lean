import BernoulliSection10Complex.AffineLog
...
/-- A dependent finite tuple of complex row groups. -/
def MultiAffineRows : List ℕ → Type
  | [] => PUnit
  | p :: ps => (Fin p → ℂ) × MultiAffineRows ps
...
@[instance_reducible] instance multiAffineRowsTopologicalSpace :
  ...
  | p :: ps =>
      ...
      inferInstanceAs
        (TopologicalSpace ((Fin p → ℂ) × MultiAffineRows ps))
...
instance multiAffineTensorNormedSpace [NormedAddCommGroup E] [NormedSpace ℂ E]
    (ps : List ℕ) : NormedSpace ℂ (MultiAffineTensor E ps) := by
  induction ps with
  | nil => exact inferInstanceAs (NormedSpace ℂ E)
  | cons p ps ih =>
      letI : NormedSpace ℂ (MultiAffineTensor E ps) := ih
      exact inferInstanceAs
        (NormedSpace ℂ
          (PiLp 2 (fun _ : Fin (p + 1) => MultiAffineTensor E ps)))
...
instance multiAffineRowsBorelSpace :
  ...
  | p :: ps =>
      ...
      inferInstanceAs
        (BorelSpace ((Fin p → ℂ) × MultiAffineRows ps))
...
def multiAffineRowLaw (μ : Measure ℂ) : (ps : List ℕ) → Measure (MultiAffineRows ps)
  | [] => Measure.dirac PUnit.unit
  | p :: ps => (Measure.pi fun _ : Fin p => μ).prod (multiAffineRowLaw μ ps)

instance multiAffineRowLawIsProbabilityMeasure (μ : Measure ℂ)
...
def multiAffineEval {E : Type*} [NormedAddCommGroup E] [NormedSpace ℂ E] :
...
theorem continuous_multiAffineEval_uncurry {E : Type*}
    [NormedAddCommGroup E] [NormedSpace ℂ E] (ps : List ℕ) :
...
row group along a complex affine line while fixing all remaining rows; the
second clause repeats the same requirement in the tail. -/
def IsMultiAffine {E : Type*} [AddCommGroup E] [Module ℂ E] :
    {ps : List ℕ} → (MultiAffineRows ps → E) → Prop
  | [], _ => True
  | p :: ps, F =>
      (∀ (tail : MultiAffineRows ps) (x y : Fin p → ℂ) (t : ℂ),
        F ((1 - t) • x + t • y, tail) =
          (1 - t) • F (x, tail) + t • F (y, tail)) ∧
      (∀ x : Fin p → ℂ, IsMultiAffine (fun tail ↦ F (x, tail)))
...
theorem map_eq_affineValue_of_line
  ...
  let D : (Fin p → ℂ) → E := fun x ↦ F x - F 0
  have hD_zero : D 0 = 0 := by simp [D]
  have hD_smul (t : ℂ) (x : Fin p → ℂ) : D (t • x) = t • D x := by
    have h := hF 0 x t
    simp only [smul_zero, zero_add] at h
    dsimp [D]
    rw [h]
    module
  have hD_affine (x y : Fin p → ℂ) (t : ℂ) :
      D ((1 - t) • x + t • y) = (1 - t) • D x + t • D y := by
    dsimp [D]
    rw [hF]
    module
  have hD_add (x y : Fin p → ℂ) : D (x + y) = D x + D y := by
    have harg : ((1 - (1 / 2 : ℂ)) • ((2 : ℂ) • x) +
        (1 / 2 : ℂ) • ((2 : ℂ) • y)) = x + y := by
      module
  ...
  funext x
  have hx_basis : x = ∑ i, x i • (Pi.single i 1 : Fin p → ℂ) := by
    ext i
    classical
  ...
  calc
    ...
    _ = F 0 + d x := rfl
    _ = F 0 + d (∑ i, x i • (Pi.single i 1 : Fin p → ℂ)) := by rw [← hx_basis]
...
theorem multiAffineTensorOfFunction_smul
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℂ E]
    (ps : List ℕ) (a : ℂ) (F : MultiAffineRows ps → E) :
...
theorem multiAffineTensorOfFunction_sum
  ...
  | empty =>
      simpa using multiAffineTensorOfFunction_smul ps (0 : ℂ)
        (fun _ : MultiAffineRows ps ↦ (0 : E))
  | @insert i s hi ih =>
      simp only [Finset.sum_insert hi]
      rw [multiAffineTensorOfFunction_add, ih]

/-- The canonical construction as a complex linear map. -/
def multiAffineTensorOfFunctionLinearMap
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℂ E]
    (ps : List ℕ) : (MultiAffineRows ps → E) →ₗ[ℂ] MultiAffineTensor E ps where
...
theorem multiAffineHeadEval_tensorOfFunction
  ...
  simpa [affineValue] using congrFun
    (map_eq_affineValue_of_line (fun u : Fin p → ℂ ↦ F (u, tail))
      (hF.1 tail)) x
...
theorem multiAffineEval_log_lintegral_abs_le
  ...
  | cons p ps ih =>
      ...
      let νp : Measure (Fin p → ℂ) := Measure.pi fun _ : Fin p => μ
      let νtail : Measure (MultiAffineRows ps) := multiAffineRowLaw μ ps
      let headDiff : (Fin p → ℂ) → ℝ := fun x ↦
        Real.log ‖multiAffineHeadEval c x‖ - Real.log ‖c‖
      let tailDiff : (Fin p → ℂ) → MultiAffineRows ps → ℝ := fun x y ↦
      ...
      have htotalMeas : Measurable fun z :
        ...
        change Measurable fun z : (Fin p → ℂ) × MultiAffineRows ps ↦
          ENNReal.ofReal
            |Real.log
...
theorem multiAffineEval_ne_zero_ae_of_ne_zero
  ...
  | cons p ps ih =>
      ...
      have hprod :
          ∀ᵐ z : (Fin p → ℂ) × MultiAffineRows ps
