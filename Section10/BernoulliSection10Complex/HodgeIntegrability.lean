...
import BernoulliSection10Complex.MultiAffineSecondMoment
import BernoulliSection10Complex.PhysicalAffinity
...
theorem measurable_finRowsToMultiAffineRows (p : ℕ) :
  ...
  induction n with
  | zero =>
      change Measurable (fun _ : Fin 0 → (Fin p → ℂ) ↦ PUnit.unit)
      exact measurable_const
  | succ n ih =>
      change Measurable (fun x : Fin (n + 1) → (Fin p → ℂ) ↦
        (x 0, finRowsToMultiAffineRows p n (Fin.tail x)))
...
theorem measurable_multiAffineRowsToFinRows (p : ℕ) :
  ...
  | zero =>
      change Measurable (fun _ : PUnit ↦
        (fun i : Fin 0 => Fin.elim0 i : Fin 0 → (Fin p → ℂ)))
      exact measurable_const
  | succ n ih =>
      change @Measurable
        ((Fin p → ℂ) × MultiAffineRows (List.replicate n p))
        (Fin (n + 1) → (Fin p → ℂ))
      ...
      · have hsnd : Measurable (fun x : (Fin p → ℂ) ×
            MultiAffineRows (List.replicate n p) ↦ x.2) := measurable_snd
        have htail : Measurable (fun x : (Fin p → ℂ) ×
            MultiAffineRows (List.replicate n p) ↦
              multiAffineRowsToFinRows p n x.2) := ih.comp hsnd
        have h : Measurable (fun x : (Fin p → ℂ) ×
            MultiAffineRows (List.replicate n p) ↦
...
def finRowsMultiAffineRowsMeasurableEquiv (p n : ℕ) :
    (Fin n → (Fin p → ℂ)) ≃ᵐ MultiAffineRows (List.replicate n p) where
...
theorem finRowsMultiAffineRows_measurePreserving
    (μ : Measure ℂ) [IsProbabilityMeasure μ] (p : ℕ) :
...
theorem isMultiAffine_comp_multiAffineRowsToFinRows
    ...
    ∀ {n : ℕ} (F : (Fin n → (Fin p → ℂ)) → E),
      (∀ (x : Fin n → (Fin p → ℂ)) (i : Fin n)
        (u v : Fin p → ℂ) (t : ℂ),
