...
in the paper.  Each group is uniformly padded to `3W` complex atom coordinates;
...
abbrev PacketAtomRows (W : ℕ) :=
  Fin (PacketAtomRowCount W) → Fin (PacketAtomRowCount W) → ℂ

def packetAtomRowsLaw (W : ℕ) (μ : Measure ℂ) : Measure (PacketAtomRows W) :=
...
def packetAtomAssignment (W : ℕ) (x : PacketAtomRows W) :
  ...
  (blockNormalization W : ℂ) *
    x (packetIndexEquiv W e.1.1) (packetIndexEquiv W e.1.2)

def packetPhysicalRowValues (W : ℕ) (i : ThreeBlockIndex (Fin W))
    (u : Fin (PacketAtomRowCount W) → ℂ) : PacketPhysicalRow i → ℂ :=
  fun e ↦ (blockNormalization W : ℂ) *
    u (packetIndexEquiv W e.1.1.2)

theorem packetAtomAssignment_update
    (W : ℕ) (x : PacketAtomRows W)
    (i : Fin (PacketAtomRowCount W))
    (u : Fin (PacketAtomRowCount W) → ℂ) :
...
theorem packetPhysicalRowValues_interpolate
    ...
    (u v : Fin (PacketAtomRowCount W) → ℂ) (t : ℂ) :
...
theorem packetAtomAssignment_rowsOfRealAssignment
    ...
    packetAtomAssignment W (packetAtomRowsOfRealAssignment W a) =
      fun e ↦ (a e : ℂ) := by
  funext e
  have hb : (blockNormalization W : ℂ) ≠ 0 := by
    exact_mod_cast blockNormalization_ne_zero W hW
  simp only [packetAtomAssignment, packetAtomRowsOfRealAssignment,
    Equiv.symm_apply_apply, e.2, dite_true]
  push_cast
  field_simp
