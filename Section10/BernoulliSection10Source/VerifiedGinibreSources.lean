...
import ShortRingAnchor.BC12.GaussianPairNormalization
...
theorem normalizedGaussianPair_map {N : ℕ} (hN : 0 < N) :
  ...
  have hatom : circularGaussianAtom = Complex.measurableEquivRealProd.symm := by
    funext p
    simp only [circularGaussianAtom, Complex.measurableEquivRealProd_symm_apply,
      Complex.mk_eq_add_mul_I, mul_comm]
  rw [hatom]
  exact BC12.normalizedGaussianPair_map hN
...
theorem ginibre_negative_on_sequence_of_bbv
  ...
  · intro n
    change (∫ x, ‖circularGaussianAtom x‖ ^ 3 ∂circularGaussianPairLaw) +
