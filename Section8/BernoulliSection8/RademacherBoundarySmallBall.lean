...
def rademacherBoundaryWidthThreshold (cook : CookDeformedSquareInput) (z : ℂ) : ℕ :=
  max (terminalCanonicalLargeWThreshold cook 1 (cook.subgaussianBound : ℝ))
    (max 1 (Nat.ceil (3 * ‖z‖ ^ 2)))

theorem packetRowScale_shift_bound (W : ℕ) (z : ℂ)
    (hW : 0 < W) (hz : 3 * ‖z‖ ^ 2 ≤ (W : ℝ)) :
...
theorem isUnit_det_smul {W : ℕ} (sigma : ℂ) (hsigma : sigma ≠ 0)
    (A : Matrix (Fin W) (Fin W) ℂ) (hA : IsUnit A.det) : IsUnit (sigma • A).det := by
  rw [Matrix.det_smul]
  exact (isUnit_iff_ne_zero.mpr (pow_ne_zero _ hsigma)).mul hA

def rademacherBoundaryBaseLoss (cook : CookDeformedSquareInput) (W : ℕ) (z : ℂ) : ℝ :=
  terminalUniformBaseLoss cook 1 (packetRowScale W * z) (rademacherPacketFamily W) (W : ℝ)

def rademacherBoundaryBadProbability (cook : CookDeformedSquareInput) (W : ℕ) : ℝ :=
  terminalUniformBadProbability cook W 1 (W : ℝ)

theorem rademacherBoundaryWidthThreshold_pos (cook : CookDeformedSquareInput) (z : ℂ) :
    0 < rademacherBoundaryWidthThreshold cook z := by
  have h : 1 ≤ rademacherBoundaryWidthThreshold cook z :=
    (le_max_left _ _).trans (le_max_right _ _)
  omega

theorem rademacherBoundaryBaseLoss_nonneg (cook : CookDeformedSquareInput) (W : ℕ) (z : ℂ) :
    0 ≤ rademacherBoundaryBaseLoss cook W z :=
  terminalUniformBaseLoss_nonneg cook 1 _ _ _

theorem rademacherBoundaryBadProbability_nonneg (cook : CookDeformedSquareInput) (W : ℕ) :
    0 ≤ rademacherBoundaryBadProbability cook W := by
  ...
  unfold rademacherBoundaryBadProbability terminalUniformBadProbability uniformCookFailureBound
  positivity
...
def rademacherBoundarySmallBall
    ...
    TerminalSmallBallConclusion (packetAtomRowsLaw W rademacherLaw)
  ...
  have hnormz : 3 * ‖z‖ ^ 2 ≤ (W : ℝ) := by
    exact (Nat.le_ceil _).trans (by
      exact_mod_cast ((le_max_right _ _).trans ((le_max_right _ _).trans hW) :
        Nat.ceil (3 * ‖z‖ ^ 2) ≤ W))
  have hshift := packetRowScale_shift_bound W z hW0 hnormz
  have hsub : ((rademacherPacketFamily W).subgaussianParameter : ℝ) ≤
      (cook.subgaussianBound : ℝ) := by
    simpa using (show (1 : ℝ) ≤ (cook.subgaussianBound : ℝ) by
      exact_mod_cast cook.subgaussianBound_one_le)
  let C := packetTerminalConcreteConclusion cook (packetAtomRowsLaw W rademacherLaw)
    1 (packetRowScale W * z) (rademacherPacketFamily W) (W : ℝ)
      (packetTerminalCanonicalLargeWConditions_of_ge_threshold cook 1
        ...
        (packetRowScale W * z) (rademacherPacketFamily W) hsub hsub hthreshold hshift)
  have hsigma := packetRowScale_ne_zero W hW0
  have hraw := BoundaryAssembly.literalCoordinateTerminalTheorem_of_packet
    (rademacherPacketFamily W) (packetRowScale W * z)
    ((packetRowScale W) • CL) ((packetRowScale W) • BR)
    (isUnit_det_smul _ hsigma CL hCL) (isUnit_det_smul _ hsigma BR hBR)
    (rademacherBoundaryBaseLoss cook W z) (rademacherBoundaryBadProbability cook W)
...
def rademacherBoundaryInverseGamma (W : ℕ) (z : ℂ)
    (CL BR : Matrix (Fin W) (Fin W) ℂ) : ℝ :=
  ‖(packetRowScale W)⁻¹ ^ (3 * W)‖ *
    (packetEndpointComparisonConstant (packetRowScale W * z)
      ((packetRowScale W) • CL) ((packetRowScale W) • BR))⁻¹

theorem rademacherBoundaryInverseGamma_pos (W : ℕ) (hW : 0 < W) (z : ℂ)
    (CL BR : Matrix (Fin W) (Fin W) ℂ) :
    0 < rademacherBoundaryInverseGamma W z CL BR := by
...
theorem rademacherPacketBoundaryCoefficient_gram_lower
    (W : ℕ) (hW : 0 < W) (z : ℂ)
    (CL BR : Matrix (Fin W) (Fin W) ℂ) (hCL : IsUnit CL.det) (hBR : IsUnit BR.det)
    (Theta : Matrix (Fin W ⊕ Fin W) (Fin W ⊕ Fin W) ℂ) (hTheta : IsUnit Theta.det) :
    rademacherBoundaryInverseGamma W z CL BR * gramVolume Theta ≤
  ...
  simpa only [Fintype.card_fin, rademacherBoundaryInverseGamma,
