...
theorem rademacherOpNormBound_one_le : (1 : ℝ) ≤ 40 * Real.sqrt 2 := by
  have hs : (1 : ℝ) ≤ Real.sqrt 2 := Real.one_le_sqrt.mpr (by norm_num)
  linarith

theorem packetRowScale_norm_bounds (W : ℕ) (hW : 0 < W) :
...
theorem rademacherScaledEndpointGood
    ...
    PaperEndpointGood ((packetRowScale W) • CL) ((packetRowScale W) • BR)
      (3 * W * (40 * Real.sqrt 2)) (interfaceDeterminantLowerBound I W) := by
  have hs := packetRowScale_norm_bounds W hW
...
theorem rademacherEndpointLogConstant_nonneg (I : NguyenBottomSingularInput) :
  ...
  unfold rademacherEndpointLogConstant
  positivity
...
theorem log_scaled_endpointExteriorConstant_le
    ...
    Real.log (endpointExteriorConstant ((packetRowScale W) • CL)
        ((packetRowScale W) • BR)) ≤
      rademacherEndpointLogConstant I * W * Real.log (Real.exp 1 * W) := by
  let D := max 0 (nguyenInterfaceDetLoss I (nguyenInterfaceCutoffRho I))
  let B := 40 * Real.sqrt 2
  let F := (24 * B * (W : ℝ) ^ 3) ^ (2 * W)
  have hD : 0 ≤ D := le_max_left _ _
  have hB : 1 ≤ B := rademacherOpNormBound_one_le
  have hW1 : (1 : ℝ) ≤ W := by exact_mod_cast (Nat.succ_le_of_lt hW)
  ...
  have hs := rademacherScaledEndpointGood I W hW CL BR hgood
...
theorem neg_log_rademacherBoundaryInverseGamma_eq
  ...
  unfold rademacherBoundaryInverseGamma
...
theorem neg_log_rademacherBoundaryInverseGamma_le
    ...
    (hgood : PaperEndpointGood CL BR (40 * Real.sqrt 2)
      (interfaceDeterminantLowerBound I W)) :
    -Real.log (rademacherBoundaryInverseGamma W z CL BR) ≤
      rademacherBoundaryLogConstant I z * W * Real.log (Real.exp 1 * W) := by
  have hs := packetRowScale_norm_bounds W hW
  ...
  have he := log_scaled_endpointExteriorConstant_le I W hW CL BR hgood
  ...
  have hcard : Fintype.card (ThreeBlockIndex (Fin W)) = 3 * W := by
    ...
    omega
  rw [neg_log_rademacherBoundaryInverseGamma_eq W hW]
...
theorem neg_log_rademacherBoundaryInverseGamma_le_on_endpoint_good
    ...
    -Real.log (rademacherBoundaryInverseGamma W z
      ...
      rademacherBoundaryLogConstant I z * W * Real.log (Real.exp 1 * W) :=
  neg_log_rademacherBoundaryInverseGamma_le I W hW z _ _
    (rademacherEndpointGoodEvent_spec I W ep hep)
...
theorem rademacherPacketBoundaryCoefficient_log_gramVolume_le
  ...
  have habsScale : |Real.log ‖(packetRowScale W)⁻¹ ^ (3 * W)‖| =
    ...
    have hlogsigma := Real.log_nonneg (packetRowScale_norm_bounds W hW).1
  ...
  calc
    ...
    _ = -Real.log (rademacherBoundaryInverseGamma W z CL BR) :=
      (neg_log_rademacherBoundaryInverseGamma_eq W hW z CL BR).symm
...
theorem rademacherPacketBoundaryCoefficient_log_gramVolume_le_on_endpoint_good
    ...
    |Real.log (rademacherPacketBoundaryCoefficient W z
      ...
      Real.log (gramVolume Theta)| ≤
      rademacherBoundaryLogConstant I z * W * Real.log (Real.exp 1 * W) :=
  rademacherPacketBoundaryCoefficient_log_gramVolume_le I W hW z _ _
    (rademacherEndpointGoodEvent_spec I W ep hep) Theta hTheta
