...
theorem normalizedShiftLogDet_rademacherMatrix (W s : ℕ) (z : ℂ) (x : ℕ → ℝ) :
    normalizedShiftLogDet (rademacherMatrix W s x) z =
      densityCyclicLogDet W s z (physicalRowsFromSequence W s x) /
        (((s + 3) * W : ℕ) : ℝ) := by
  unfold normalizedShiftLogDet densityCyclicLogDet rademacherMatrix
...
theorem rademacher_log_converges_iff_physical_rows
    (W s : ℕ → ℕ) (z : ℂ) (u : ℝ) :
    TendstoInMeasure rademacherSequenceLaw
      (fun n x => normalizedShiftLogDet (rademacherMatrix (W n) (s n) x) z)
      atTop (fun _ => u) ↔
  ...
  simpa only [TendstoInProbabilityTri, tendstoInMeasure_iff_measureReal_norm,
    Real.norm_eq_abs, ← normalizedShiftLogDet_rademacherMatrix] using h

theorem rademacher_high_band_rows_log_potential
  ...
  (rademacher_log_converges_iff_physical_rows W s z _).1
    (rademacher_high_band_log_potential hSource W s hW hs hWtop ω hω hωlt hband z)

/-- The independent anchor's normalized actual cyclic determinant. -/
def anchorLogPotential (W : ℕ) (z : ℂ) (x : IntervalRows W (anchorSites W)) : ℝ :=
...
theorem rademacher_anchor_log_potential
    (hSource : Section3SubgaussianHighBandInput rademacherLaw 1)
    ...
    TendstoInProbabilityTri (fun n => intervalRowsLaw (W n) (anchorSites (W n)) rademacherLaw)
      (fun n => anchorLogPotential (W n) z) (circularLogPotential z) := by
...
def directOrAnchorCoreSites (W s : ℕ) : ℕ :=
  if (s + 3) * W < anchorSize W then s else anchorCells W * cellSites W

theorem directOrAnchorCoreSites_pos {W s : ℕ} (hW : 0 < W) (hs : 0 < s) :
    0 < directOrAnchorCoreSites W s := by
  unfold directOrAnchorCoreSites
  split
  · exact hs
  · exact Nat.mul_pos (anchorCells_pos hW) (cellSites_pos W)

theorem eventually_directOrAnchor_highBand (W s : ℕ → ℕ)
    (hWtop : Tendsto W atTop atTop) :
    ∀ᶠ n in atTop,
      (((directOrAnchorCoreSites (W n) (s n) + 3) * W n : ℕ) : ℝ) ^
        (8 / 9 + 1 / 20 : ℝ) ≤ W n := by
...
def directBranchLogPotential (W s : ℕ) (z : ℂ) (x : IntervalRows W (s + 3)) : ℝ :=
...
theorem rademacher_direct_branch_log_potential
    (hSource : Section3SubgaussianHighBandInput rademacherLaw 1)
    (W s : ℕ → ℕ) (hW : ∀ n, 0 < W n) (hs : ∀ n, 0 < s n)
    (hWtop : Tendsto W atTop atTop) (z : ℂ) :
    TendstoInProbabilityTri (fun n => intervalRowsLaw (W n) (s n + 3) rademacherLaw)
      (fun n => directBranchLogPotential (W n) (s n) z) (circularLogPotential z) := by
  have h := rademacher_high_band_rows_log_potential hSource W
    (fun n => directOrAnchorCoreSites (W n) (s n)) hW
    (fun n => directOrAnchorCoreSites_pos (hW n) (hs n)) hWtop
  ...
  · let F : ℕ → ℝ := fun q =>
    ...
    have hsites : directOrAnchorCoreSites (W n) (s n) = s n := by
