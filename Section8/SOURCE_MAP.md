...
This source-level audit records the exact paper obligations. Their
implementation correspondence and verified build status are recorded in
...
zero. These modules passed the scoped normal build at source commit
`15433d8`; see `FORMALIZATION_MAP.md` for the declaration and verification ledger.
...
## Final assembly crosswalk: verified

The following map identifies the concrete declarations for (8.52)-(8.69).
...
File names below are relative to `Section8/BernoulliSection8`, and
declaration names are in namespace `BernoulliSection8` unless indicated otherwise.

| PDF obligation | Verified declarations and exact role | Status |
| --- | --- | --- |
...
tail. The intentionally external Cook, Nguyen, and Section 3 inputs remain
explicit assumptions after verification. The scoped normal build, strict
28-declaration axiom audit, and public-signature audit all passed at
`15433d8` in CI run 33676900277. A clean `#print axioms` report alone does
not show that a theorem has no additional parameters; both axiom and
signature checks were completed.
