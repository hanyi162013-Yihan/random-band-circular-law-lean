...
The literal random matrices used in Sections 3--5 have a different finite product
...
/-- The calibration bridge in the exact strength used by Section 5: an `L¹` comparison
...
/-- Two-step calibration bridge matching the intended Section 4 adapter shape.  The
...
comparisons from literal Section 4 expressions is a separate obligation. -/
...
/-- Section 4's final seam and maximal-pressure fluctuation bounds, together with the
Section 5 deterministic mean-pressure limit, imply the long-branch log-potential limit.
...
/-- Select the Section 3 short branch or the Section 4--5 long branch at each index. -/
